...
{
    uint8_t sock = ESP8266::getFreeSocket();
    if (sock != NO_SOCKET_AVAIL)
    {
        EspDrv::startClient("0", port, sock, UDP_MODE);
		
        ESP8266::allocateSocket(sock);  // allocating the socket for the listener
        ESP8266::_server_port[sock] = port;
        _sock = sock;
        _port = port;
        return 1;
    }
...
void WiFiEspUDP::stop()
{
      ...
      ESP8266::_state[_sock] = NA_STATE;
      ESP8266::_server_port[_sock] = 0;
